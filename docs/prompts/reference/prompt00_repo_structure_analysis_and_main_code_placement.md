# Prompt 00 鈥?Repository Structure Analysis and Main Code Placement
...
1. **News Processing**  
   Raw Top-20 financial news 鈫?structured event/view/sector/regime signals.

2. **Quantified Text Storage Medium**  
   Structured text signals 鈫?event table, view matrix, belief state, retrieval index, or knowledge graph.

3. **Trade Data Processing**  
   Official price/portfolio data 鈫?returns, volatility, covariance, momentum, drawdown, turnover capacity, and risk state.

4. **Final Trading Agent**  
   Text state + trade state 鈫?target weights / official executable trades.
